...
from tabl import Tabl, read_tabl, T, first
import os
import pytest
import numpy as np
...
@pytest.fixture
def tbls():
    tbls = [Tabl({'Name' : ["John", "Joe", "Jane"], 'Height' : [1.82,1.65,2.15], 'Married': [False,False,True]}),
            Tabl([ np.array(["John", "Joe", "Jane"]),np.array([1.82,1.65,2.15]),np.array([False,False,True])], columns = ["Name", "Height", "Married"]),
            Tabl([ ["John", "Joe", "Jane"],[1.82,1.65,2.15],[False,False,True]], columns = ["Name", "Height", "Married"]),
            Tabl(),
            Tabl([1,2,3]),
            Tabl({'a':range(10),'b':range(10)}),
            Tabl({'a':range(10),'b':list(map(str, range(10)))}),
            Tabl([range(10),range(10),range(10)]),
            Tabl(np.array([np.arange(10),np.arange(10,20)])),
            Tabl({'a':12,'b':13}),
            Tabl(pd.DataFrame({'a':[12]*6, 'b':np.arange(6), 'c':range(6)})),
            Tabl({'a':[3,4,5], 'b':1}),
            Tabl({'a':1, 'b':[3,4,5]}),
            Tabl({'aa':[1,1,1], 'b':[3,4,5]}),
            ]
    return tbls


class TestTablInit(object):
    def test_tabl_init(self, tbls):
        for tbl in tbls:
            assert tbl.valid


class TestAppend(object):
    def test_append_tabl(self, tbls):
        for tbl in tbls:
            length = len(tbl)
            tbl += tbl
            assert tbl.valid
    ...
    def test_append_combinations(self):
        first_tbl = [Tabl(), Tabl({'a':2,'b':3}), Tabl({'a':"ff",'b':"ffr"})]
        seccond_tbl = [Tabl({'a':22,'b':13}), pd.DataFrame({'a':[22],'b':[13]}),
                       Tabl({'a':"ff",'b':"ffr"}),
    ...
    def test_slice_type(self, tbls):
        for tbl in tbls:
            if len(tbl) > 0:
                ...
                assert type(tbl[:,:]) == Tabl
                assert type(tbl[0,0]) == type(tbl.data[0][0])
                assert type(tbl[[0],[0]]) == Tabl
                (r,c) = tbl.shape
                assert type(tbl[np.array([True]*r),np.array([True]*c)]) == Tabl
                assert type(tbl[list(range(r)),list(range(c))]) == Tabl
                assert type(tbl[:,:]) == Tabl
...
class TestSetter(object):
    ...
    def test_set_row(self):
        tbl = Tabl({"a":list(range(2,6)), "b": ['1','2'] *2, "c":[1.1,2.2]*2})
        tbl[0,:] = (12, "12", 12.12)
        tbl[1,['a','b']] = [12, "12"]
        tbl[2] = (13, "13", 13.13)
            ...
            for fmt in ['csv', 'gz', 'npz']:
                for tbl in tbls:
                    if len(tbl) > 0:
                        ...
                        tbl_r = read_tabl(fn, header=header)
                        ...
                        if not tbl_r.columns[0].isdigit() and len(tbl)>2:
                            ...
                            tbl_r = read_tabl(fn, header=None)
...
class TestJoin(object):
    ...
    def test_outer_join(self):
        tbl = Tabl({"a":list(range(4)), "b": ['a','b'] *2})
        tbl_b = Tabl({"a":list(range(2,6)), "c": [1,2] *2, "d":[1.1,2.2]*2})
...
class TestDTypeProperties(object):
    def test_dtype_properties(self):
        tbl = Tabl({"a":list(range(2,6)), "c": [u'1',u'2'] *2, "d":[1.1,2.2]*2})
...
class TestDelItem(object):
    def test_del_row_int(self):
        tbl1 = Tabl({"a":list(range(2,6)), "b": ['1','2'] *2, "c":[1.1,2.2]*2})
        tbl2 = Tabl({"a":list(range(2,6)), "b": ['1','2'] *2, "c":[1.1,2.2]*2})
        del tbl1[0]
        assert len(tbl1) == 3
